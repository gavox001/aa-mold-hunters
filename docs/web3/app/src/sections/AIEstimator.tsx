...
export function AIEstimator() {
  ...
  const calculateEstimate = () => {
    ...
    let base = (damageType?.factor || 2000);
    let multiplier = (propertyType?.factor || 1) * (areaSize?.factor || 1) * (urgency?.factor || 1);
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-eco-primary/20 flex items-center justify-center">
                    <currentQuestion.icon className="w-6 h-6 text-eco-primary" />
                  </div>
